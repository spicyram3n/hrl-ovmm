...
grasp       -> core/perception/grasping/grasp_planner.py for the pose,
               ik_solver_node/solve_ik_with_collision for arm IK, then
               /arm_trajectory_controller/joint_trajectory and
...
from core.perception.grasping import grasp_planner
...
class HsrRobotAdapter(Node):
    def __init__(self):
        super().__init__('hsr_robot_adapter')
        ...
        self.ik_client = self.create_client(
            SolveIkWithCollision, 'ik_solver_node/solve_ik_with_collision')
    ...
    def navigate_to(self, pose: dict) -> bool:
        ...
        goal.pose.orientation.z = pose['qz']
        goal.pose.orientation.w = pose['qw']
    ...
    def look_at(self, point_map: list[float]) -> None:
        ...
        pt.point.x, pt.point.y, pt.point.z = float(point_map[0]), float(point_map[1]), float(point_map[2])
    ...
    def grasp(self, object_pcd) -> bool:
        pose = grasp_planner.compute_grasp_pose(object_pcd)
        cx, cy, cz = pose['position_camera']
        ...
        try:
            pt_odom = self.tf_buf.transform(pt, ODOM_FRAME, timeout=rclpy.duration.Duration(seconds=1.0))
        except Exception as e:
            self.get_logger().error(f'grasp: TF to {ODOM_FRAME} failed ({e})')
            return False

        if not self.ik_client.wait_for_service(timeout_sec=5.0):
            self.get_logger().error('grasp: ik_solver_node service unavailable '
                                     '(launch fetcher ik_solver.launch.py)')
            return False

        req = SolveIkWithCollision.Request()
        req.origin_to_hand_goal.position.x = pt_odom.point.x
        req.origin_to_hand_goal.position.y = pt_odom.point.y
        req.origin_to_hand_goal.position.z = pt_odom.point.z + pose['pregrasp_height']
        qx, qy, qz, qw = pose['orientation']
        req.origin_to_hand_goal.orientation.x = qx
        req.origin_to_hand_goal.orientation.y = qy
        req.origin_to_hand_goal.orientation.z = qz
        req.origin_to_hand_goal.orientation.w = qw
        ...
        if not future.done() or not future.result().ik_results:
            self.get_logger().error('grasp: IK solver returned no solutions')
            return False

        ik = future.result().ik_results[0]
        self.get_logger().info(f'[grasp] IK solution: {list(zip(ik.joint_names, ik.joint_positions))}')

        self._send_gripper(GRIPPER_OPEN)
        self.spin_for(GRIPPER_SETTLE_SECS)

        self._send_arm(ik.joint_names, ik.joint_positions)
        ...
        lift_positions = list(ik.joint_positions)
        if 'arm_lift_joint' in ik.joint_names:
            i = ik.joint_names.index('arm_lift_joint')
            lift_positions[i] = min(lift_positions[i] + LIFT_DELTA, 0.69)
        self._send_arm(ik.joint_names, lift_positions)
        self.spin_for(ARM_SETTLE_SECS)
